...
//analog "_cur_read_chain"
int _cur_read_node(Cursor* cur, off_t offset)
{
    cur->prev = cur->node;
    cur->prevoff = cur->nodeoff;
    cur->nodeoff = offset;
    lseek(cur->fh, cur->nodeoff, SEEK_SET);
...
// analog "_cur_cmp_chain"
int _cur_cmp_node(Cursor* cur, const char* key) 
{
    size_t ks = strlen(key) + 1;
    log("Comparing key '%s' with cursor\n", key);
    if ( cur->node.keysz != ks ) return 0;
...
// analog "_cur_find"
int _cur_find_node(Cursor* cur, const char* key) 
{
    ...
    if ( cur->node.next )
    {
        ...
        _cur_read_node(cur, cur->node.next);

        return _cur_find_node(cur, key);
    }
    return 0;
}
...
int _cur_set_value(Cursor* cur, const char* value) {
    ...
    _file_append_block(cur->fh, value, &cur->node.valueoff, &cur->node.valuesz);
    _cur_update_node(cur);
    cur->stat->valuesz += cur->node.valuesz - oldsz;
    _cur_update_stat(cur);
}
//--my--
//TODO
...
int ht_get(DB* db, const char* key, char** value) {
    ...
    if ( _ht_search(db, &cur, key) )
    {
        *value = malloc(cur.node.valuesz);
        ...
        if ( read(cur.fh, *value, cur.node.valuesz) == cur.node.valuesz )
            return 1;
        else
        {
            error("Cannod read %lu bytes for value at %ld\n", cur.node.valuesz, cur.node.valueoff);
            return 0;
        }
    }
    return 0;
